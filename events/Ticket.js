...
module.exports = {
    ...
    execute(interaction, client) {
      db.query(`SELECT * FROM ticket WHERE guildId = "${interaction.guild.id}"`, async (err, req) => {
        ...
        const category_Direction = req[0].category_Direction;
        const category_Professeur = req[0].category_Professeur;
            ...
            interaction.guild.channels.create(`ticket-${interaction.user.username}`, {
                ...
                collector.on('collect', i => {
                    if (i.user.id === interaction.user.id) {
                        ...
                        if (i.values[0] == 'Direction') {
                          c.edit({
                            parent: category_Direction
                          });
                        };
                        if (i.values[0] == 'Professeur') {
                          c.edit({
                            parent: category_Professeur
                          });
                        };
                        if (i.values[0] == 'autre') {
                          c.edit({
        ...
        if (interaction.customId == "delete-ticket") {client
            const guild = client.guilds.cache.get(interaction.guildId);
            const chan = guild.channels.cache.get(interaction.channelId);
