...
exports.run = async (client, message, args) => {
    ...
    let user = message.user ? client.users.cache.get(args._hoistedOptions[0].value) : (message.mentions.users.first() || client.users.cache.get(args[0].value));
    ...
    try {
        await user.send(`<@${message.author.id}> vous à rendu la parole sur le serveur **${message.guild.name}**.`)
    } catch (err) {}

    message.guild.members.cache.get(user.id).timeout(null, `${reason} (Parole rendu par <@${message.author.id}>)`)
    message.channel.send(`<@${message.author.id}> à rendu la parole à **${user.tag}**.`)
